 ...
 *
 * Verdicts:
 *   correct → green   close → amber   wrong → muted
 * Numeric attributes also carry a direction arrow telling the player whether
 * the secret movie's value is higher (up) or lower (down) than their guess.
 */
import type { AttributeResult, GuessResult, MovieFacts } from "@/lib/types";
import { formatMoney, formatRating } from "@/lib/format";

const YEAR_CLOSE = 5; // within ±5 years → amber
const RATING_CLOSE = 0.5; // within ±0.5 points → amber (green only on an exact match)
const REVENUE_CORRECT = 0.1; // within 10% → green
const REVENUE_CLOSE = 0.4; // within 40% → amber

function compareYear(guess: MovieFacts, target: MovieFacts): AttributeResult {
  const g = guess.year;
  const t = target.year;
 ...
 */
function compareRating(guess: MovieFacts, target: MovieFacts): AttributeResult {
...
/**
 * Genres are scored per-genre with no "close" tier: each genre the guess
 * shares with the target is a green match, the rest are grey. The overall
 * verdict is green only when the two genre sets are identical.
 */
function compareGenres(guess: MovieFacts, target: MovieFacts): AttributeResult {
...
function compareStudio(guess: MovieFacts, target: MovieFacts): AttributeResult {
...
function compareCast(guess: MovieFacts, target: MovieFacts): AttributeResult {
...
function compareDirector(guess: MovieFacts, target: MovieFacts): AttributeResult {
...
export function compareFacts(guess: MovieFacts, target: MovieFacts): GuessResult {
  return {
    guess,
