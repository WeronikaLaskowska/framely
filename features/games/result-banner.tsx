...
export function ResultBanner({ won, target, detail, onPlayAgain }: ResultBannerProps) {
  const poster = tmdbImage(target.posterPath, "w342");
  return (
    <div className="fr-card fr-pop relative overflow-hidden p-6 md:p-8">
        ...
        <div className="flex flex-col">
          <span
            className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-widest ${
              won
                ? "bg-fr-correct/15 text-fr-correct"
                : "bg-fr-close/15 text-fr-close"
            }`}
          >
          ...
          <h2 className="fr-display mt-3 text-3xl md:text-4xl">{target.title}</h2>
          <p className="mt-1 text-fr-fg-muted">
            {target.year ?? "—"} · {target.genres.map((g) => g.name).join(", ")}
          </p>
          {detail && <p className="fr-counter mt-1">{detail}</p>}
          ...
          <button
            type="button"
            onClick={onPlayAgain}
            className="fr-btn fr-btn--ember mt-6 w-fit px-6 py-3"
          >
